...
| `percussion` | Percussion | The other light page. Black type, set properly and perfectly still — scrolling does nothing to it. Tap it and it detonates where you touched: the glyphs near the strike take an impulse away from it, hardest at the centre and scattered enough not to be a tidy ring, tumbling as they go. Every one is then on a damped spring back to exactly where it belongs, so the paragraph reassembles itself over `Heal` seconds. Springs rather than a timed animation, so a second tap while the first is still settling adds to whatever velocity a glyph already has and the two blasts interfere. Rotation springs to the nearest whole turn, so a glyph most of the way round finishes the turn instead of rewinding through it. | Canvas; window pointer listeners that only watch. |
...
assets/
  ...
  styles.css
  profiles/
    tidewater.js  lantern.js  ledger.js  marbles.js  percussion.js  foundry.js
posts/
